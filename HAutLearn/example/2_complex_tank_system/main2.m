...
addpath(['HAutLearn', filesep, 'example', filesep, '2_complex_tank_system', filesep, 'trainingdata']);
...
reduced_trace = trace([1,3,4,5,7,8,9,10]);

for n =1:length(reduced_trace)
    %reduced_trace(n).labels_num = 1:5;
    reduced_trace(n).labels_trace = [reduced_trace(n).labels_trace;0];
end

ode = FnEstODE(reduced_trace);
t2 = toc;

%% LI Estimation given parameters
tic;
[reduced_trace,label_guard] = FnLI(reduced_trace, eta, lambda, gamma);
t3 = toc;

%% Setup PTA given LIs and ODEs
pta_trace = FnPTA(reduced_trace);
pta_trace = pta_filter(pta_trace);
t_train = toc;

%% Generate Final Automaton model

FnGenerateHyst(['HAutLearn', filesep, 'example', filesep, '2_complex_tank_system', filesep, 'automata_learning'],label_guard, num_var, ode, pta_trace);

addpath(['HAutLearn', filesep, 'src',filesep,'hyst', filesep, 'src', filesep, 'matlab']);

try
    SpaceExToStateflow(['HAutLearn', filesep, 'example', filesep, '2_complex_tank_system', filesep, 'automata_learning.xml']);
catch
...
function indx = changepoint(values)
    ...
    indx = find(diffs<=-0.05|diffs>=0.05)+len;
