...
addpath(['ProposedAlgorithm', filesep, 'example', filesep, 'Dev2']);
...
fixedIntervalLength = 2; precisionDTL = 0.01; useTime = true; %used for DTL
...
subplot(2,1,2);
ylim([0 1]);
plot(offsetPred+fixedIntervalLength*(0:(size(sim_x,1)-1)),sim_x(:,2),1:size(trace(1).x,1),trace(1).x(:,2));
hold off;