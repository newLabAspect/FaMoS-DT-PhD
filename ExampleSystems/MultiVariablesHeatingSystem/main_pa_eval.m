...
num_var = 2; num_ud = 0; %general paras
...
if(length(rangePrecision) ~= 1)
    writematrix([rangePrecision',falseAll],['ExampleSystems', filesep, 'MultiVariablesHeatingSystem', filesep, 'MVHS_quant_time.csv']);
elseif (Nsteps ~= 1)
    writematrix([round(linspace(1,size(X,1),Nsteps),0)',falseAll],['ExampleSystems', filesep, 'MultiVariablesHeatingSystem', filesep, 'MVHS_size_time_smallquant.csv']);
end