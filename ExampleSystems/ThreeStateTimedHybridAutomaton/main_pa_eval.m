...
addpath(['ExampleSystems', filesep, 'ThreeStateTimedHybridAutomaton']);
...
if(length(rangePrecision) ~= 1)
    writematrix([rangePrecision',falseAll],['ExampleSystems', filesep, 'ThreeStateTimedHybridAutomaton', filesep, '3STHA_quant_time.csv']);
elseif (Nsteps ~= 1)
    writematrix([round(linspace(1,size(X,1),Nsteps),0)',falseAll],['ExampleSystems', filesep, 'ThreeStateTimedHybridAutomaton', filesep, '3STHA_size_time_smallquant.csv']);
end