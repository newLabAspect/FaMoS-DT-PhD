...
addpath(['ExampleSystems', filesep, 'ThreeStateStochasticHybridAutomaton']);
...
sigma = 0.01;  winlen=5; thresClusterMin = 0.015; thresClusterMax = 0.1; offsetCluster = 1; facThres = 2.5; %used in clustering
Nsteps = 1; rangePrecision = 0.001; %linspace(0.01,0.0001,500); %used in eval
...
if(length(rangePrecision) ~= 1)
    writematrix([rangePrecision',falseAll],['ExampleSystems', filesep, 'ThreeStateStochasticHybridAutomaton', filesep, '3SPHA_quant_time.csv']);
elseif (Nsteps ~= 1)
    writematrix([round(linspace(1,size(X,1),Nsteps),0)',falseAll],['ExampleSystems', filesep, 'ThreeStateStochasticHybridAutomaton', filesep, '3SPHA_size_time.csv']);
end