...
addpath(['ExampleSystems', filesep, 'ThreeStateHybridAutomaton']);
...
sigma = 0.01;  winlen=5; thresClusterMin = 0.01; thresClusterMax = 0.1; offsetCluster = 1; facThres = 2.5; %used in clustering
...
if(length(rangePrecision) ~= 1)
    writematrix([rangePrecision',falseAll],['ExampleSystems', filesep, 'ThreeStateHybridAutomaton', filesep, '3SHA_quant_time.csv']);
elseif (Nsteps ~= 1)
    writematrix([round(linspace(1,size(X,1),Nsteps),0)',falseAll],['ExampleSystems', filesep, 'ThreeStateHybridAutomaton', filesep, '3SHA_size_time_smallquant.csv']);
end