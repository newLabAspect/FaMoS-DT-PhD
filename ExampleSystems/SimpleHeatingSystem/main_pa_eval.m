...
sigma = 0.01;  winlen=5; thresClusterMax = 0.1; thresClusterMin = 0.01; offsetCluster = 0; facThres = 2.5; %used in clustering
...
if(length(rangePrecision) ~= 1)
    writematrix([rangePrecision',falseAll],['ExampleSystems', filesep, 'SimpleHeatingSystem', filesep, 'SHS_quant_time.csv']);
elseif (Nsteps ~= 1)
    writematrix([round(linspace(1,size(X,1),Nsteps),0)',falseAll],['ExampleSystems', filesep, 'SimpleHeatingSystem', filesep, 'SHS_size_time_smallquant.csv']);
end